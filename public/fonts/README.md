...
picked in Settings via `clockFont`. Self-hosted here instead of linked from
Google Fonts, matching this app's offline-first rule (no runtime dependency
on an external CDN).
...
The three newer files are subset to just digits + colon (the only glyphs the
clock ever renders) via Google's `text=` subsetting parameter, so each is a
few KB.
